...
const Flight = () => {
  ...
  const saveFlightDetails = async () => {
    const totalFootprint = calculateFlightCarbonFootprint(flightClass, parseFloat(flightHours));
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        try {
          ...
          console.log('Flight details saved to Firestore for the current month');
//          navigate('/home');
        } catch (error) {
  ...
  return (
    <div>
      ...
      <div>
        ...
        <select value={flightClass} onChange={(e) => handleFlightClassChange(e.target.value)}>
      ...
      <div>
        ...
        <input type="number" value={flightHours} onChange={(e) => handleFlightHoursChange(e.target.value)} />
      </div>
      <div>
